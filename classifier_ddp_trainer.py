...
from configs.cls_train_cfg import Config
...
def run_one_epoch(cfg, is_train, epoch_iter, model, data_loader, criterion, optimizer, device, logger, viser, rank):
    ...
    with torch.set_grad_enabled(is_train):
        while data is not None:
            ...
            # print(output.shape, label_tensor.shape)
            ...
            acc1 = 100.0 * (pred_output == gt_output).sum() / len(pred_output)
            losses.update(loss.item(), img_tensor.size(0))
            top1.update(acc1, img_tensor.size(0))
...
def main_worker():
    ...
    if cfg.dataloader_type == 'file':
        # 0负样本 1正样本
        from dataloaders.ClassifierImageFileData import ImageFileDataset as MyDataset
    ...
    train_dataset = MyDataset(cfg.train_file_path, data_transforms['train'], class_num=cfg.class_num, class_map=cfg.class_map)
    ...
    test_dataset = MyDataset(cfg.test_file_path, data_transforms['test'], class_num=cfg.class_num, class_map=cfg.class_map)
