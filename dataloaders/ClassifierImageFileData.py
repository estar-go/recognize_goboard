import os, sys, numpy as np, random, torch, cv2
import torch.utils.data as data
from glob import glob

from tqdm import tqdm
import torchvision.transforms as transforms
from PIL import Image

class ImageFileDataset(data.Dataset):
    def __init__(self, base_data_path, transform, class_map, class_num, image_postfix='.png'):
        print('data init')
        self.base_data_path=base_data_path
        self.transform = transform
        self.class_map = class_map
        self.class_num = class_num
        self.image_postfix = image_postfix

        if os.path.isdir(base_data_path):
            self.fnames = self.get_data_list(base_data_path)
        elif os.path.isfile(base_data_path):
            print("暂不支持!")
            exit()        

    def get_data_list(self, base_data_path):
        file_list = glob(base_data_path + f'*/*/*{self.image_postfix}') + glob(base_data_path + f"*/*/*.jp*")
        
        return file_list

    def __getitem__(self, idx):
        # 返回 图片 和对应的 类别 0负样本 1正样本
        now_image_file = self.fnames[idx]
        
        img = Image.open(now_image_file).convert("RGB")
        img = self.transform(img)
        
        label_name = now_image_file.split('/')[-3]
        label = 0
        
        if label_name not in self.class_map:
            print(f"{now_image_file} 类别有问题！")
        else:
            label = self.class_map[label_name]

        return img, label

    def __len__(self):
        return len(self.fnames)


if __name__ == '__main__':
    np.set_printoptions(threshold=sys.maxsize)
    transform = transforms.Compose([
        transforms.Resize((604, 604)),
        transforms.ToTensor(),
        transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
    ])
    datasets = ImageFileDataset(base_data_path='/data/datasets/godatasets/goboard_datasets/test/', transform=transform)
    print(len(datasets))
    it = 0
    for feature, label in datasets:
        # print(feature)
        # nl = label.numpy()
        # print(nl)
        # cv2.imshow('feature', feature)
        # if cv2.waitKey(1000000) & 0xFF == ord('q'):
        #     break
        # print(feature.shape, label.shape)
        pass
        
        it += 1
        if it > 3:
            break