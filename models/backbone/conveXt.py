...
if __name__ == '__main__':
    ...
    # model = PPLCNet_x1_0(pretrained='.pretrained/PPLCNet_x1_0_ssld_pretrained.pth')
    depths = [3, 3, 3]
    dims=[192, 384, 768]
    dims=[96, 192, 384]
    depths = [3, 3, 9, 3]
    ...
    dims = [48, 96, 192, 384]
    class_num = 3
    model = convnext_tiny(depths=depths, dims=dims, class_num=class_num)
    # model = convnext_tiny()
    ...
    for i in tqdm(range(n)):
        y = model(input)
    b = time.time()
    print(f'cost time: {1000 * (b-a) / n :.5f} ms.')
    print(y.shape)
    # for x in y:
    #     print(x.shape)
